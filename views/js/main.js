...
function updatePositions() {
  ...
  var items = movingPizzas;  // Cached globally
...
// Also cache moving pizzas globally
var movingPizzas;
document.addEventListener('DOMContentLoaded', function () {
  var cols = 8;
  var s = 256;
  var rows = Math.ceil(window.screen.height / s);  // Dynamic rows based on screen height
  ...
  for (var i = 0; i < numPizzas; i++) {
    ...
    elem.style.transform = 'translateX(' + elem.basicLeft + 'px)';  // Initial position via transform
    document.querySelector("#movingPizzas1").appendChild(elem);
  }

  movingPizzas = document.querySelectorAll('.mover');  // Cache here
  updatePositions();
});